%% Post-traitement Welch — simul2 (chaîne complète)
%
%   acc_mesuree : signal de sortie de la chaîne (g, fs_dec = 500 Hz)
%   fs          : fréquence d'échantillonnage en entrée de chaîne (5000 Hz)
%   fs_dec      : fréquence d'échantillonnage en sortie du décimateur (500 Hz)

%% Estimation DSP par Welch sur acc_mesuree (fs_dec = 500 Hz)
%   Nyquist après décimation = 250 Hz
%   N_fft = 500 → df = fs_dec/N_fft = 500/500 = 1 Hz  (bins à 1, 2, 3 Hz...)
%   N_fft = 1000 → df = 0.5 Hz (bins à 0.5, 1.0, 1.5 Hz...)
N_fft_dec  = 1000;              % df = 0.5 Hz sur le signal décimé
...
%% g_rms mesuré (bande 4-200 Hz, cohérente avec la normalisation init2.m)
bande        = f_w >= 4 & f_w <= 200;
...
%% Affichage
fig = figure('Name', 'Post-traitement simul2 — chaîne idéale');
